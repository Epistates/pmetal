...
impl LlamaAttention {
    ...
    pub fn forward_with_cache(
        ...
        // cache, and a cached decode never has explicit positions.
...
impl AttentionModule for LlamaAttention {
    fn forward_with_cache(
        ...
        LlamaAttention::forward_with_cache(self, x, mask, cache, positions)
    }
}
...
impl DecoderLayer for LlamaDecoderLayer {
    fn forward_with_cache(
        ...
        LlamaDecoderLayer::forward_with_cache(self, x, mask, cache, positions)
    }
}
...
impl LlamaForCausalLM {
    ...
    /// [`LlamaModel::forward_with_positions`].
    pub fn forward_with_positions(
        &mut self,
        input_ids: &Array,
        mask: Option<&Array>,
    ...
    ///
    /// Runs one `[N_active, 1]` forward across every active slot,
    /// reusing the shared [`FusedBatchKVCache`] for all layers. Shape
    /// contract matches [`crate::dispatcher::DynamicModel::forward_batched`].
