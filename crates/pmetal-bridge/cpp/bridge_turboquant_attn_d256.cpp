...
// Phase E.4 V2: outlier-bias variant of d256 base no_qjl_2pass.
// Same structure as the d128 + d256-fullbyte siblings — adds one global
// memory load + add per (row, slot). The bias is `[q_rows,
// cache_seq_capacity]` f32 indexed by the q-row, indexed alongside the
// kernel's existing slot loop.
static const char* TURBOQUANT_ATTENTION_Q8_D256_NO_QJL_2PASS_1_WITH_OUTLIER_BIAS_SOURCE = R"(
    constexpr uint kDim = 256u;
    constexpr uint kVec = 8u;
    constexpr uint kKeyCentroids = 256u;
    constexpr uint kValueCentroids = 256u;
...
get_turboquant_attention_q8_d256_no_qjl_2pass_1_with_outlier_bias_kernel() {
    static auto kernel = mlx::core::fast::metal_kernel(
        "turboquant_attention_q8_d256_no_qjl_2pass_1_with_outlier_bias",
        {
            "query_rot",
            "key_indices",
            "key_norms",
        ...
        {"partials", "sums", "maxs"},
        TURBOQUANT_ATTENTION_Q8_D256_NO_QJL_2PASS_1_WITH_OUTLIER_BIAS_SOURCE,
        "",
        true,
        false
    );
...
int mlx_inline_turboquant_attention_q8_d256_no_qjl_2pass_with_outlier_bias(
    mlx_inline_array*       out,
    const mlx_inline_array* query_rot,
    const mlx_inline_array* key_indices,
    const mlx_inline_array* key_norms,
    ...
    uint32_t                attn_scale_bits)
{
    ...
    try {
        ...
        auto& pass1 = get_turboquant_attention_q8_d256_no_qjl_2pass_1_with_outlier_bias_kernel();
        auto pass1_outputs = pass1(
            {
                query_rot_arr,
                key_indices_arr,
        ...
        new (out->buf) array(pass2_outputs[0]);
        return 0;
    } catch (const std::exception& e) { pmetal_bridge_set_last_error("turboquant_attention_q8_d256_no_qjl_2pass_with_outlier_bias", e.what()); return 1; } catch (...) { pmetal_bridge_set_last_error("turboquant_attention_q8_d256_no_qjl_2pass_with_outlier_bias", "unknown C++ exception"); return 1; }
}
