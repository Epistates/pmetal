...
impl LlamaQloraForCausalLM {
    ...
    pub fn with_qlora_config(config: LlamaConfig, qlora_config: QLoraConfig) -> Result<Self, LoraError> {
...
mod tests {
    ...
    fn test_qlora_zero_lora_initial() {
        ...
        let config = small_config();
        let qlora_config = small_qlora_config();
        let mut model = LlamaQloraForCausalLM::with_qlora_config(config.clone(), qlora_config).unwrap();
